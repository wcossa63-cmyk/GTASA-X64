...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, ImGuiButtonFlags flags)
{
   ...
   // const ImVec2 padding = g.Style.FramePadding;
    const ImRect bb(GetCursorPos(), GetCursorPos() + image_size);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
    ...
    // Render
    if(pressed || held)
        window->DrawList->AddImage(texture_id, bb.Min, bb.Max, uv0, uv1, ImColor(0x92, 0xC4, 0x91, 255));
    else
        window->DrawList->AddImage(texture_id, bb.Min, bb.Max, uv0, uv1);
