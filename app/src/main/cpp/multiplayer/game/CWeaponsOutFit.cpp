...
void CWeaponsOutFit::OnWeaponAdded(CPedSamp* pPed, int iWeaponID)
{
	int iModelID = CUtil::GameGetWeaponModelIDFromWeaponID(iWeaponID);
	if (iModelID == -1)
	{
		return;
	}
	...
	info.dwBoneId_MP = GetSampIDFromInternalBoneID(m_Settings[iWeaponID].iBone);
	...
	//CChatWindow::AddMessage("ATTACH slot %d %f %f %f %d", iSlot + 10, info.vecOffset.x, info.vecOffset.y, info.vecOffset.z, info.dwBone);
}

void CWeaponsOutFit::OnWeaponRemoved(CPedSamp* pPed, int iWeaponID)
{
	...
	//CChatWindow::AddMessage("DETACH slot %d", iSlot + 10);
}

void CWeaponsOutFit::OnPedDie(CPedSamp* pPed)
{
...
void CWeaponsOutFit::ProcessLocalPlayer(CPedSamp* pPed)
{
