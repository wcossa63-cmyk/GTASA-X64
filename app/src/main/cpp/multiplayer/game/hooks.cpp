...
#include "CLoader.h"
//Samsung FIX
#include "Samsung/gl_compat.h"
#include "Samsung/gl_buffer_safe.h"
#include "Samsung/NavigationPauseFix.h"
...
stFile* NvFOpen(const char*, const char* filename, bool, bool)
{
    snprintf(lastFile, sizeof(lastFile), "%s", filename);

	char path[512]{};

	snprintf(path, sizeof(path), "%s%s", g_pszStorage, filename);

	if (strstr(filename, "mainV1.scm"))
	{
		snprintf(path, sizeof(path), "%sSAMP/main.scm", g_pszStorage);
		Log("Loading %s", path);
	}
	if (strstr(filename, "SCRIPTV1.IMG"))
	{
		snprintf(path, sizeof(path), "%sSAMP/script.img", g_pszStorage);
		Log("Loading script.img..");
	}
	if (!strncmp(filename, "DATA/VEHICLES.IDE", 17))
	{
		snprintf(path, sizeof(path), "%sSAMP/vehicles.ide", g_pszStorage);
		Log("Loading vehicles.ide..");
	}

	if (!strncmp(filename, "DATA/GTA.DAT", 12))
	{
		snprintf(path, sizeof(path), "%sSAMP/gta.dat", g_pszStorage);
		Log("Loading gta.dat..");
	}

	if (!strncmp(filename, "DATA/HANDLING.CFG", 17))
	{
		snprintf(path, sizeof(path), "%sSAMP/handling.cfg", g_pszStorage);
		Log("Loading handling.cfg..");
	}

	if (!strncmp(filename, "DATA/WEAPON.DAT", 15))
	{
		snprintf(path, sizeof(path), "%sSAMP/weapon.dat", g_pszStorage);
		Log("Loading weapon.dat..");
	}

	auto *st = (stFile*)malloc(sizeof(stFile));
...
void InjectHooks()
{
...
#if VER_x32
    CHook::WriteMemory(g_libGTASA + 0x324F8E, (uintptr_t)"\x15\xE0", 2);
#else
    CHook::WriteMemory(g_libGTASA + 0x3ED380, (uintptr_t)"\x11\x00\x00\x14", 4);
#endif
    CHook::RET("_ZN8CGridRef4InitEv");
    CHook::RET("_ZN9CShopping9LoadStatsEv");
	...
	CDraw::InjectHooks();
	CClock::InjectHooks();
	CHook::RET("_ZN6CBirds6UpdateEv");
	CHook::RET("_ZN6CBirds6RenderEv");
	CHook::RET("_ZN6CBirds19CreateNumberOfBirdsE7CVectorS0_iib");
...
void CPedDamageResponseCalculator__ComputeDamageResponse_hook(stPedDamageResponse* thiz, CEntity* pEntity, uintptr_t pDamageResponse, bool bSpeak)
{
	...
	float originalDamage = thiz->fDamage;
	...
	if(!pLocalPed || !pLocalPed->m_bIsSpawned || pLocalPed->IsDead()) {
		CPedDamageResponseCalculator__ComputeDamageResponse(thiz, pEntity, pDamageResponse, bSpeak);
		return;
	}
...
void CEntity__Render_hook(CEntity* entity) {
    ...
    if (entity->IsObject() && pNetGame && CLocalPlayer::GetPlayerPed()) {
...
void RQCommand_rqSetAlphaTest(uint8_t **qData) {
	*qData += 8; // next command
}
...
void InstallHooks()
{
	...
	auto glAlphaFuncQCOM = eglGetProcAddress("glAlphaFuncQCOM");
	if (!glAlphaFuncQCOM)
	{
		glAlphaFuncQCOM = eglGetProcAddress("glAlphaFunc");
		if(glAlphaFuncQCOM)
			*(uintptr_t *) (g_libGTASA + (VER_x32 ? 0x6BCBF8 : 0x89A1B0)) = reinterpret_cast<uintptr_t>(glAlphaFuncQCOM);
		else
