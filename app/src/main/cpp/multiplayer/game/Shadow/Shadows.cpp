...
void CShadows::StoreCarLightShadow(CVehicleSamp* vehicle, int32 id, RwTexture* texture, CVector* posn, float frontX, float frontY, float sideX, float sideY, uint8 red, uint8 green, uint8 blue, float maxViewAngleCosine) {
    ...
    auto pVeh = vehicle->m_pVehicle;
    if (pVeh)
    {
        vehicle->ProcessHeadlightsColor(red, green, blue);
        red     /= 4;
        green   /= 4;
        blue    /= 4;
