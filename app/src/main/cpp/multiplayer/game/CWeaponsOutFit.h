...
class CWeaponsOutFit
{
	...
	static void OnWeaponAdded(CPedSamp* pPed, int iWeaponID);
	static void OnWeaponRemoved(CPedSamp* pPed, int iWeaponID);
	static void OnPedDie(CPedSamp* pPed);
public:
	static void ParseDatFile();
	static void ProcessLocalPlayer(CPedSamp* pPed);
