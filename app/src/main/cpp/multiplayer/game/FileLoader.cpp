...
void CFileLoader::LoadPedModels() {
    std::ifstream input(std::string(g_pszStorage) + "data/ped_models.json");
    if (!input.is_open())
        return;
    ...
    for (const auto& model : models) {
        const int32 modelId = model["id"].get<int32>();
        if (modelId < 0 || modelId >= CModelInfo::NUM_MODEL_INFOS || CModelInfo::GetModelInfo(modelId))
            continue;

        const std::string modelName = model["modelName"].get<std::string>();
        const std::string txdName = model["txdName"].get<std::string>();
        const std::string animGroup = model["animGroup"].get<std::string>();
        const std::string animFile = model["animFile"].is_string()
            ? model["animFile"].get<std::string>()
            : "null";

        snprintf(
            line,
            sizeof(line),
            "%d %s %s CIVMALE STAT_STREET_GUY %s 0 0 %s 9 9 PED_TYPE_GEN VOICE_GEN_NOVOICE VOICE_GEN_NOVOICE",
            modelId,
            modelName.c_str(),
            txdName.c_str(),
            animGroup.c_str(),
        ...
        CTxdStore::FindOrAddTxdSlot(txdName.c_str(), "skins");
        CHook::CallFunction<int32>("_ZN11CFileLoader13LoadPedObjectEPKc", line);
    }
}
