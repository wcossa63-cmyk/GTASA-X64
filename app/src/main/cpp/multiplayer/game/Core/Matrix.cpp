...
void CMatrix::RotateFromDegrees(CVector& rot) {
    if (rot.x != 0.0f)
        RwMatrixRotate((RwMatrix*)this, CVector::XAxisVector(), rot.x);

    if (rot.y != 0.0f)
		RwMatrixRotate((RwMatrix*)this, CVector::YAxisVector(), rot.y);

    if (rot.z != 0.0f)
		RwMatrixRotate((RwMatrix*)this, CVector::ZAxisVector(), rot.z);
}
