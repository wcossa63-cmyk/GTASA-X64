...
void ApplyFixRoad()
{
    uintptr_t func = g_libGTASA + (VER_x32 ? 0x1E7612 : 0x283454);

    mprotect((void*)(func & ~0xFFF), 0x1000, PROT_READ | PROT_WRITE | PROT_EXEC);

#if VER_x32
    // mov r0, #1; bx lr
    memcpy((void*)func, "\x01\x20\x70\x47", 4);
#else

    // mov w0, #1; ret
    memcpy((void*)func, "\x20\x00\x80\x52\xC0\x03\x5F\xD6", 8);
#endif

    __builtin___clear_cache((char*)func, (char*)(func + 8));
    Log("BlackRoadFix: applied");
}
...
void ApplyPatches()
{
    ...
    CHook::RET("_ZN15CAEStreamThread9PlayTrackEjijcbb");
