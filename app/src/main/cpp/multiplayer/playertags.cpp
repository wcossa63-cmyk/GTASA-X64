    ...
    for (auto & pair : CActorPool::list) {
        ...
        if (distFromCam <= pNetGame->m_fNameTagDrawDistance) {
            ...
            if (!pNetGame->m_bNameTagLOS || CWorld::GetIsLineOfSightClear(vecPos, CCamera::Get().GetPosition(), false, false, false, false, false, false, false)) {
                ...
                CVector screenPos;
                if(!CSprite::CalcScreenCoors(vecPos, &screenPos, nullptr, nullptr, false, true))
                    continue;

                static char szNameBuf[255];
                memset(szNameBuf, 0, sizeof(szNameBuf));
                snprintf(szNameBuf, sizeof(szNameBuf), "%s", pActor->m_szName);

                if (strlen(szNameBuf) == 0)
                    continue;

                const auto textSize = ImGui::CalcTextSize(szNameBuf);
                ImVec2 pos(screenPos.x - textSize.x / 2, screenPos.y);

                float health = pActor->m_pPed->m_fHealth;
                if (health > 0.0f) {
                    if (health > 100.0f) health = 100.0f;

                    const float fWidth = pGUI->ScaleX(60.0f);
                    const float fHeight = pGUI->ScaleY(10.0f);
                    const float fOutline = 1.0f;

                    ImVec2 outline1 = ImVec2(screenPos.x - fWidth / 2 - fOutline, pos.y + pGUI->GetFontSize() + 2.0f - fOutline);
                    ImVec2 outline2 = ImVec2(screenPos.x + fWidth / 2 + fOutline, pos.y + pGUI->GetFontSize() + 2.0f + fHeight + fOutline);
                    ImGui::GetForegroundDrawList()->AddRectFilled(outline1, outline2, IM_COL32(0, 0, 0, 255));

                    ImVec2 barBg1 = ImVec2(screenPos.x - fWidth / 2, pos.y + pGUI->GetFontSize() + 2.0f);
                    ImVec2 barBg2 = ImVec2(screenPos.x + fWidth / 2, barBg1.y + fHeight);
                    ImGui::GetForegroundDrawList()->AddRectFilled(barBg1, barBg2, IM_COL32(40, 40, 40, 255));

                    float healthWidth = fWidth * (health / 100.0f);
                    ImVec2 bar1 = ImVec2(screenPos.x - fWidth / 2, barBg1.y);
                    ImVec2 bar2 = ImVec2(bar1.x + healthWidth, barBg2.y);
                    ImGui::GetForegroundDrawList()->AddRectFilled(bar1, bar2, IM_COL32(185, 34, 40, 255));
                }
            }
        }
    }
...
void CPlayerTags::Draw(CVector* tagPos, const char* szName, const float *distFromCam, CRemotePlayer* pPlayer)
{
    tagPos->z += 0.25f + (*distFromCam * 0.0475f);

    CVector Out;
    if(!CSprite::CalcScreenCoors(*tagPos, &Out, nullptr, nullptr, false, true))
        return;
    ...
    DrawHealthAndArmour(&Out, pPlayer->m_fCurrentHealth, pPlayer->m_fCurrentArmor);
