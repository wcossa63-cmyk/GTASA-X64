...
void RequestClass(RPCParameters *rpcParams)
{
    ...
    constexpr int expectedBits = (1 + sizeof(PLAYER_SPAWN_INFO)) * 8;
    if (!rpcParams || rpcParams->numberOfBitsOfData < expectedBits)
    {
        Log("RPC RequestClass: invalid payload size %d bits",
            rpcParams ? rpcParams->numberOfBitsOfData : 0);
        return;
    }

    RakNet::BitStream bsData(rpcParams->input,
                             (rpcParams->numberOfBitsOfData / 8) + 1, false);

    uint8_t outcome = 0;
    PLAYER_SPAWN_INFO spawnInfo{};
    bsData.Read(outcome);
    bsData.Read(reinterpret_cast<char*>(&spawnInfo), sizeof(spawnInfo));

    if (outcome != 0)
    {
        CLocalPlayer::SetSpawnInfo(spawnInfo);
        CLocalPlayer::HandleClassSelectionOutcome();
    }
}
...
void RequestSpawn(RPCParameters *rpcParams)
{
    ...
    if (!rpcParams || rpcParams->numberOfBitsOfData < 32)
    {
        Log("RPC RequestSpawn: invalid payload size %d bits",
            rpcParams ? rpcParams->numberOfBitsOfData : 0);
        return;
    }
    ...
    uint32_t outcome = 0;
    bsData.Read(outcome);
    CLocalPlayer::HandleSpawnRequestReply(outcome);
}
...
void WorldPlayerAdd(RPCParameters *rpcParams)
{
	...
	if(pRemotePlayer)
	{
		pRemotePlayer->Spawn(byteTeam,
							 ...
							 dwColor,
							 byteFightingStyle,
							 CRemotePlayer::eTags::NONE
		);
		pRemotePlayer->m_bShowNameTag = showNameTag;
	}
}
...
void WorldVehicleAdd(RPCParameters *rpcParams)
{
	...
	if(CHook::CallFunction<int>(g_libGTASA + (VER_x32 ? 0x3863EC + 1 : 0x45D0C8), NewVehicle.iVehicleType) == -1) {
            CChatWindow::DebugMessage("Error: try create vehicle with not valid vehicle model %d", NewVehicle.iVehicleType);
		return;
	}
	...
	Log::addParameter("NewVehicle.byteAddSiren", NewVehicle.byteAddSiren);
	Log::addParameter("NewVehicle.byteInterior", NewVehicle.byteInterior);
	Log::addParameter("NewVehicle.aColor1", NewVehicle.aColor1);
	Log::addParameter("NewVehicle.aColor2", NewVehicle.aColor2);
	Log::addParameter("NewVehicle.byteLightDamageStatus", NewVehicle.byteLightDamageStatus);
	Log::addParameter("NewVehicle.byteTireDamageStatus", NewVehicle.byteTireDamageStatus);
	Log::addParameter("NewVehicle.dwDoorDamageStatus", NewVehicle.dwDoorDamageStatus);
	Log::addParameter("NewVehicle.dwPanelDamageStatus", NewVehicle.dwPanelDamageStatus);
	Log::addParameter("NewVehicle.fHealth", NewVehicle.fHealth);
	Log::addParameter("NewVehicle.fRotation", NewVehicle.fRotation);
	Log::addParameter("NewVehicle.iVehicleType", NewVehicle.iVehicleType);
	Log::addParameter("NewVehicle.vecPos.x", NewVehicle.vecPos.x);
	Log::addParameter("NewVehicle.vecPos.y", NewVehicle.vecPos.y);
	Log::addParameter("NewVehicle.vecPos.z", NewVehicle.vecPos.z);
	Log::addParameter("NewVehicle.VehicleID", NewVehicle.VehicleID);

	Log::addParameter("NewVehicle.byteModSlots", NewVehicle.byteModSlots);
	Log::addParameter("NewVehicle.bytePaintjob", NewVehicle.bytePaintjob);
	Log::addParameter("NewVehicle.aColor1", NewVehicle.cColor1);
	Log::addParameter("NewVehicle.aColor2", NewVehicle.cColor2);
...
void GameModeRestart(RPCParameters *rpcParams)
{
	...
	CChatWindow::AddMessage("{ff0000}Соединение прервано сервером...");
...
if(byteRejectReason == REJECT_REASON_BAD_VERSION)
	...
	pNetGame->SetGameState(eNetworkState::DISCONNECTED);
}
...
void WorldPlayerDeath(RPCParameters* rpcParams)
{
	...
	if (!rpcParams || rpcParams->numberOfBitsOfData < sizeof(PLAYERID) * 8)
		return;
	...
	if (CRemotePlayer* player = CPlayerPool::GetAt(playerId))
		player->HandleDeath();
}
