...
bool CVehiclePool::New(NewVehiclePacket *pNewVehicle) {
    ...
    if (pNewVehicle->aColor1 != 0xFF || pNewVehicle->aColor2 != 0xFF) {
        pVeh->SetColor(pNewVehicle->aColor1, pNewVehicle->aColor2);
    }
...
bool CVehiclePool::Delete(VEHICLEID VehicleID)
{
	if (auto pVehicle = GetAt(VehicleID)) {
		...
		for (auto it = rwObjectToIdMap.begin(); it != rwObjectToIdMap.end();) {
			if (it->second == VehicleID)
				it = rwObjectToIdMap.erase(it);
			else
				++it;
		}
