...
void CLocalPlayer::Init() {
    ...
    m_dwLastSendSpecTick = now;
    m_dwLastAimSendTick = now;
    m_dwLastStatsUpdateTick = now;
    m_dwLastUpdateHudButtons = now;
    m_dwPassengerEnterExit = now;
...
void CLocalPlayer::ResetSpawnState()
{
    memset(&m_SpawnInfo, 0, sizeof(m_SpawnInfo));
    m_iSelectedClass = 0;
    ...
    lastDamageId = INVALID_PLAYER_ID;
    lastDamageWeap = WEAPON_UNARMED;

    if (m_pPlayerPed)
        m_pPlayerPed->m_bIsSpawned = false;
}
...
bool CLocalPlayer::Process()
{
    ...
    if (!m_bIsActive)
    {
        if (m_bIsWasted)
        {
            const uint8_t action = m_pPlayerPed->GetActionTrigger();
            if (action != ACTION_WASTED && action != ACTION_DEATH)
            {
                m_bIsWasted = false;
                if (m_bWantsAnotherClass || !m_bClearedToSpawn)
                {
                    m_bWantsAnotherClass = false;
...
void CLocalPlayer::SendDeath()
{
	...
	bsPlayerDeath.Write(static_cast<uint8_t>(lastDamageWeap));
	bsPlayerDeath.Write(static_cast<uint16_t>(lastDamageId));
...
void CLocalPlayer::GoEnterVehicle(bool passenger)
{
	...
	if (ClosetVehicleID != INVALID_VEHICLE_ID)
	{
		...
		if (pVehicle != nullptr && pVehicle->m_pVehicle->GetDistanceFromLocalPlayerPed() < 9.0f)
		{
...
{
    if (!pNetGame || pNetGame->GetGameState() != eNetworkState::CONNECTED)
        return;
...
{
    if (!m_bHasSpawnInfo || !m_pPlayerPed || !m_pPlayerPed->m_pPed)
        return;
...
{
    if (!pNetGame || pNetGame->m_iSpawnsAvailable <= 0)
        return;

    m_iSelectedClass = (m_iSelectedClass + 1) % pNetGame->m_iSpawnsAvailable;

    if (m_pPlayerPed && m_pPlayerPed->m_pPed)
    {
        const CVector& pos = m_pPlayerPed->m_pPed->GetPosition();
        CGame::PlaySound(1052, pos.x, pos.y, pos.z);
    }
...
void CLocalPlayer::SendPrevClass()
{
    ...
    {
        const CVector& pos = m_pPlayerPed->m_pPed->GetPosition();
        CGame::PlaySound(1053, pos.x, pos.y, pos.z);
    }
...
{
    if (!pNetGame || pNetGame->GetGameState() != eNetworkState::CONNECTED ||
        !m_bHasSpawnInfo || !m_bClearedToSpawn || m_bWaitingForSpawnRequestReply)
        return;
...
void CLocalPlayer::RequestClass(int32_t classId)
{
    if (!pNetGame || pNetGame->GetGameState() != eNetworkState::CONNECTED)
        return;
    ...
    request.Write(classId);
    pNetGame->GetRakClient()->RPC(&RPC_RequestClass, &request, HIGH_PRIORITY, RELIABLE, 0,
...
void CLocalPlayer::RequestSpawn()
{
    ...
    RakNet::BitStream request;
    pNetGame->GetRakClient()->RPC(&RPC_RequestSpawn, &request, HIGH_PRIORITY, RELIABLE, 0,
                                  false, UNASSIGNED_NETWORK_ID, nullptr);
}

void CLocalPlayer::SetSpawnInfo(const PLAYER_SPAWN_INFO& spawnInfo)
{
    m_SpawnInfo = spawnInfo;
    m_bHasSpawnInfo = true;
}

bool CLocalPlayer::HandleSpawnRequestReply(uint32_t outcome)
{
    const bool shouldSpawn = outcome == 2 || (outcome != 0 && m_bWaitingForSpawnRequestReply);
    m_bWaitingForSpawnRequestReply = false;
    return shouldSpawn && Spawn();
}
...
bool CLocalPlayer::Spawn()
{
    ...
    if (!pNetGame || pNetGame->GetGameState() != eNetworkState::CONNECTED ||
        !m_bHasSpawnInfo || !m_pPlayerPed || !m_pPlayerPed->m_pPed)
    {
        Log("CLocalPlayer::Spawn rejected: connected=%d, spawnInfo=%d, ped=%p",
            pNetGame && pNetGame->GetGameState() == eNetworkState::CONNECTED,
            m_bHasSpawnInfo, m_pPlayerPed);
        return false;
    }
    ...
    FindPlayerInfo().PlayerState = PLAYERSTATE_PLAYING;
    ...
    m_pPlayerPed->m_pPed->SetInterior(0, true);
    m_byteCurInterior = 0;
    ...
    CVector spawnPosition = m_SpawnInfo.vecPos;
    spawnPosition.z += 0.5f;
    if(m_pPlayerPed->m_pPed->IsInVehicle())
        m_pPlayerPed->m_pPed->RemoveFromVehicleAndPutAt(spawnPosition);
    else
        m_pPlayerPed->m_pPed->Teleport(spawnPosition, false);
...
void CLocalPlayer::SendAimSyncData()
{
    ...
    AIM_SYNC_DATA aimSync = {
            ...
            .m_bKeyboardOpened = CKeyBoard::m_bEnable // LR FIX
    };
